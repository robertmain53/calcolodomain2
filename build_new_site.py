...
print("AVVIO SCRIPT DI RICOSTRUZIONE TOTALE DA ZERO (v4)")
...
<meta name="google-site-verification" content="IL_TUO_CODICE_GSC" />
...
# File di dati (I nostri "cervelli")
SITEMAP_FILE = os.path.join(BASE_DIR, "sitemap_data_FINAL.json")
CONTENT_FILE = os.path.join(BASE_DIR, "content_data.json")

# Nomi dei file Guscio
...
def load_data_files():
    ...
    try:
        ...
        print("2. 'sitemap_data_FINAL.json' (da enrich_sitemap)")
        print("3. 'content_data.json' (da extract_content)")
...
def generate_other_schemas():
    ...
    return json.dumps(org), json.dumps(web)

# --- 3. ESECUZIONE PRINCIPALE (LA FABBRICA v4) ---
def main():
    ...
    # Itera sul DATABASE DI CONTENUTO (la nostra fonte di verità)
    for url_path, content in content_data.items():
        
        try:
            # Scegli il guscio (template) corretto
            if url_path == "/":
                current_template = template_homepage
                relative_path = "index.html"
            else:
                current_template = template_interna
                # Ricostruisci il path (es. /finance/mortgage -> finance/mortgage.html)
                relative_path = url_path[1:] + ".html"

            print(f"Costruendo: {relative_path}...")
            
            # Prendi i dati della categoria
            category, subcategory, page_title_from_map = get_page_category_info(url_path, sitemap_data)
            
            # Usa il titolo estratto (più pulito) da content_data
            page_title = content['title']
            
            # --- Inizia a Sostituire i Segnaposto ---
            ...
            output_html = output_html.replace("%%BODY_SCRIPTS_QUI%%", SCRIPT_ADSENSE_BODY)
            
            output_html = output_html.replace("%%CONTENUTO_PRINCIPALE_QUI%%", content['main_html'])
            
            # Genera e inserisci parti dinamiche
            ...
            # Combina tutti gli schemi JSON-LD
            all_schemas = [breadcrumb_schema, org_schema, web_schema]
            # TODO: Aggiungere qui la logica per generare schemi HowTo/FAQ
            output_html = output_html.replace("%%SCHEMA_JSON_LD_QUI%%", f"[{', '.join(all_schemas)}]")
            ...
            with codecs.open(output_path, 'w', 'utf-8') as f_out:
                f_out.write(output_html) # Scriviamo l'HTML pulito
    ...
    # --- 4. RIEPILOGO FINALE ---
    print("\n--- RICOSTRUZIONE DA ZERO COMPLETATA ---")
