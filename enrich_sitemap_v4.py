...
# ⚠️ INCOLLA QUI IL NOME ESATTO DEL MODELLO CHE HAI TROVATO
# (es. 'models/gemini-2.5-flash')
...
def load_map_and_find_orphans(sitemap_path, site_dir):
    ...
    for category, subcategories in sitemap_data.items():
        for subcategory, pages in subcategories.items():
            category_set.add(f"{category} > {subcategory}") # Aggiungi la stringa 'Cat > Sub'
            for page in pages:
                known_urls.add(page['url'])
    
    category_list = sorted(list(category_set)) # Ora abbiamo una lista ordinata
...
def main():
    if GOOGLE_API_KEY == "INCOLLA_LA_TUA_CHIAVE_API_QUI":
        ...
        return
    if NOME_MODELLO == "models/gemini-2.5-flash":
         print("INFO: Sto usando il modello 'models/gemini-2.5-flash'. Se dà errore 404, aggiorna NOME_MODELLO con un altro dalla tua lista.")
        ...
        for orphan_page in batch:
            ...
            if page_url in results:
                ...
                if classification not in ["IGNORE", "ERROR"]:
                    ...
                    # --- MODIFICA LOGICA DI PARSING (Più robusta) ---
                    if ' > ' in classification:
                        ...
                        # L'AI ha fallito e ha usato uno spazio (es. "US Taxation (State) Generale")
                        # Proviamo a correggerlo
                        parts = classification.rsplit(' ', 1)
                        if len(parts) == 2:
                            potential_match = f"{parts[0]} > {parts[1]}"
                            # Controlliamo se questa correzione esiste nella nostra lista di categorie
                    ...
                    # Se siamo qui, abbiamo 'category' e 'subcategory' validi
                    try:
                        page_info = {
